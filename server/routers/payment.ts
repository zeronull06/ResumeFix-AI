...
export async function createPaddleCheckout(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Paddle API error (${response.status}): ${err || response.statusText}`
      );
    }
    ...
    if (!data?.data?.checkout?.url) {
      throw new Error("Invalid Paddle response: missing checkout URL");
    }

    return data.data.checkout.url;
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to create Paddle checkout: ${errorMsg}`);
  }
}
  ...
  createCheckout: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        throw new Error(`Checkout creation failed: ${errorMsg}`);
      }
    }),
  ...
  checkPayment: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        throw new Error(`Payment check failed: ${errorMsg}`);
      }
    }),
});
