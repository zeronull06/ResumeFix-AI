...
// AI Helpers using Manus Forge (Gemini)
...
async function generateOptimizedResume(
  ...
  try {
    const improvementsList = suggestions
      .map((s) => `- [${s.section}] ${s.suggestion}`)
      .join("\n");
  ...
  extractPdf: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        throw new Error(
          `Failed to parse PDF: ${errorMsg}. Please paste your resume text manually.`
        );
      }
    }),
  ...
  saveDraft: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        throw new Error(`Failed to save draft: ${errorMsg}`);
      }
    }),

  /**
   * Run AI analysis — called after payment is confirmed.
   * Frontend calls this after Paddle webhook confirms payment.
   */
